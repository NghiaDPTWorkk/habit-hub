...
function toRecord(items: Checkin[]): Record<string, Checkin> {
  return Object.fromEntries(items.map((c) => [makeCheckinKey(c.habitId, c.date), c]))
}

// 21 days of check-ins across all habits for a realistic demo state.
export const SEED_CHECKINS: Record<string, Checkin> = toRecord([
